...
BIOS_KBD_VECTOR:	dw 0	; 16-bit offset
			dw 0	; 16-bit segment

current_color:		db 0x0f	; current color

kbdintr:
	push ax			; What happens if we fail to preserve AX?
	mov ah, [current_color]
	inc ah			; Move to the next foreground color
	and ah,0fh		; ...but always keep the background black
	mov [current_color], ah	; Update the shared variable, so our main loop notices
	pop ax

	jmp far [BIOS_KBD_VECTOR]	; The original interrupt handler will iret

start:
	; Set up interrupt handler for IRQ1 - Keyboard
	; [int 09h, see IBM PC configuration for 8259A PIC]
	xor ax, ax	; 8086 Interrupt Vector Table (IVT) is at 0000:0000
	mov es, ax	; Point ES to segment 0000
	mov si, 9h	; We are interested in position 9 in the table
	sal si, 2	; Each position holds 4 bytes
	mov dx, [es:si] 		; Fetch original offset
	mov [BIOS_KBD_VECTOR], dx	; And remember for later use
	mov dx, [es:si + 2] 			; Fetch original segment
	mov word [BIOS_KBD_VECTOR + 2], dx	; And remember for later use

	cli			; What happens if an interrupt happens while we're updating the IVT?
	mov word [es:si], kbdintr	; Replace offset in IVT with our offset
	mov [es:si + 2], ax		; Replace segment in IVT with our segment [0000]
	sti			; Critical segment ends
...
vgachar:		; Output one character to the VGA console
	...
	lodsb		; Fetch byte from DS:[SI] into AL
	mov ah, [current_color]	; AH = color attribute: Fetch *shared* byte.
